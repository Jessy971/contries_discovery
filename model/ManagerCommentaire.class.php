<?php
  ...
  class ManagerCommentaires extends BdConnection{
    ...
    {
        $creat= $this->connexionBdd->prepare(' INSERT INTO commentaire(pseudo, commentaire, date_com, pays) VALUES(:pseudo, :com, NOW(), :pays)');
        $creat->execute(
          array(
            'pseudo' => $pseudo,
            'com'    => $com,
    ...
    {
        $read = $this->connexionBdd->prepare('SELECT pseudo, commentaire, DATE_FORMAT(date_com, \'%d-%m-%Y à %Hh%i.\') AS date_com FROM commentaire WHERE pays = :pays ORDER BY id DESC');
        $read->execute(array(
          'pays'=> $pays));
        return $read;
      }
